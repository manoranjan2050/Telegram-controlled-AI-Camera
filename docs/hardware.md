...
| MicroSD power gate | The SD socket's VDD is switched by a P-MOSFET (`Q1`, AO3401) gated by **GPIO45** (net `SD1_PWRN`). **Read directly from the schematic's actual SD sheet (page 4 of 8) on 2026-09-10, not just the pin table**: `Q1` sits with source on the always-on `ESP_3V3` rail and drain feeding the card's VDD (through `C60`/`C61` decoupling) — the gate (GPIO45) has its own 51K pull-down (`R4`) to GND. For a P-channel MOSFET that means the card is powered **by default**, even before firmware ever touches GPIO45 (gate floats to 0V via R4 → Vgs very negative → ON). Driving GPIO45 low (as this firmware does) just confirms the default state; driving it high is what would turn the card *off*. **The bus also already has real 51K pull-up resistors (`R35`-`R39`) wired from the 3.3V rail to CMD/D0/D1/D2/D3** — standard SD bus pull-ups, present in hardware, not something firmware needs to add (confirms enabling `SDMMC_SLOT_FLAG_INTERNAL_PULLUP` in software was redundant, not wrong - the bus was never actually floating). This rules out both leading firmware-side theories (power sequencing, missing pull-ups) by direct circuit inspection, not just by testing. Combined with four independent live-hardware attempts (pins, host slot, power-gate polarity x2, pull-ups) all producing the identical `sdmmc_init_ocr: send_op_cond (1) returned 0x107` failure at the same point, every time — a fully powered, fully pulled-up bus that still won't answer the very first identification command points to something firmware cannot fix: **no card seated, a dead/incompatible card, or a physical defect on the socket itself** (bad solder joint, damaged pad). Next steps require physical access to the board: (1) confirm a card is actually seated in the slot; (2) try a different, known-good, small (≤32GB) FAT32-formatted MicroSD card; (3) if it still fails, probe the SD socket's pins directly with a multimeter/oscilloscope while booting. | Schematic pages 1 and 4 of 8 (direct circuit trace, not the pin table alone) + repeated live hardware tests |
