...
| MicroSD power gate | The SD socket's VDD is switched by a P-MOSFET (`Q1`, AO3401) gated by **GPIO45** (net `SD1_PWRN`), pulled down by `R4` so the card is powered by default even before firmware runs. The bus also has real 51K pull-ups (`R35`-`R39`) on CMD/D0-D3 already in hardware. Both confirmed correct by direct schematic trace (page 4/8). ✅ **SOLVED 2026-09-10 - was never a hardware or card problem.** Root cause: `telegramp4_wifi_init()` (which starts the ESP32-C6 link over SDIO, slot 1) was always called *before* `telegramp4_storage_init()` (SD, slot 0) - both share the same physical SDMMC/SDIO host controller. WiFi's SDIO init left shared host state that slot 0's own init could never recover from, producing an identical `send_op_cond (1) returned 0x107` failure no matter what else was tried (pins, power-gate polarity, pull-ups - all real, all correct, all irrelevant to the actual bug). Proven by flashing the **official DFRobot MicroPython factory test** for this exact board (from wiki.dfrobot.com/dfr1172/docs/22904, using identical pins/slot) - with WiFi never touched, the card mounted cleanly on the first try (`os.listdir('/sd')` → `['System Volume Information']`). Fix: reorder `app_main()` in `main/app_main.cpp` to init camera → SD → WiFi (see the code comment there for the full ordering rationale, including why camera has to go first too, for its own DMA channel). This also exposed a second, unrelated bug once all three subsystems could finally succeed together: `CONFIG_VFS_MAX_COUNT` (default 8, a fixed-size table of *registered VFS backends*, not open files) filled up before lwip could register its socket range, aborting with `ESP_ERR_NO_MEM` - raised to 16 in `sdkconfig.defaults`. | Schematic pages 1 and 4 of 8 + official DFRobot MicroPython factory test + live hardware verification |
